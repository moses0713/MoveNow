...
namespace Mediapipe.Unity.HandTracking
{
    public class HandTrackingSolution : ImageSourceSolution<HandTrackingGraph>
    {
        ...
        protected override IEnumerator WaitForNextValue()
        {
            ...
            Debug.Log("WaitForNextValue" + handWorldLandmarks[0].Landmark[0].X);
        ...
        private bool isProcessResult1 = false;
        private bool isProcessResult2 = false;
        float x1 = 0, y1 = 0;
        float x2 = 0, y2 = 0;
        ...
        private void OnHandLandmarksOutput(object stream, OutputEventArgs<List<NormalizedLandmarkList>> eventArgs)
        {
            ...
            {
                isProcessResult1 = false;
                isProcessResult2 = false;
        ...
        void Update()
        {
            if (isProcessResult1)
            {
                changePlayerPosition(x1, y1, player_1);
            }

            if (isProcessResult2)
            {
                changePlayerPosition(x2, y2, player_2);
            }
        }

        void changePlayerPosition(float x, float y, GameObject obj)
        {
            obj.transform.position = new Vector3(x * 1280, (1 - y) * 720, 0);
        }
